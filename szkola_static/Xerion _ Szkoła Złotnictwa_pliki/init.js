		...
		// skelJS
			skelJS: {
				...
				breakpoints: {
					...
					'normal': { range: '-1182', containers: 960 },
					'mobile': { range: '-860', lockViewport: true, containers: '95%', grid: { collapse: true } }
				}
			}
	};
					...
					if ($header.length > 0)
					{
						...
						$window
							.on('resize.overflow_fsh', function() {
								...
								{
									var p = Math.max(192, ($window.innerHeight() - $header_header.outerHeight() - $navbar.outerHeight() + $cookies.outerHeight()) / 2);
									$header.css('padding', 0.3*p + 'px 0 ' + 1.7*p + 'px 0');
								}
							})
